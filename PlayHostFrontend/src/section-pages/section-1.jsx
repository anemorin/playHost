import React from "react";

const Section = () => {
  return (
    <div className="container">
      <div className="row">
        <div className="col-lg-6">
          <div className="subtitle mb-3">Невероятные возможности</div>
          <h2 className="mb20">Премиальные игровые сервера</h2>
        </div>

        <div className="col-lg-6"></div>

        <div className="col-lg-3 col-md-6 mb-sm-20">
          <div>
            <img src="./img/icons/1.png" className="mb20" alt="" />
            <h4>Быстрая загрузка</h4>
            <p>
              Dolor minim in pariatur in deserunt laboris eu pariatur labore
              excepteur cupidatat cupidatat duis dolor in.
            </p>
          </div>
        </div>

        <div className="col-lg-3 col-md-6 mb-sm-20">
          <div>
            <img src="./img/icons/2.png" className="mb20" alt="" />
            <h4>Премиальное железо</h4>
            <p>
              Dolor minim in pariatur in deserunt laboris eu pariatur labore
              excepteur cupidatat cupidatat duis dolor in.
            </p>
          </div>
        </div>

        <div className="col-lg-3 col-md-6 mb-sm-20">
          <div>
            <img src="./img/icons/3.png" className="mb20" alt="" />
            <h4>Защита от спама</h4>
            <p>
              Dolor minim in pariatur in deserunt laboris eu pariatur labore
              excepteur cupidatat cupidatat duis dolor in.
            </p>
          </div>
        </div>

        <div className="col-lg-3 col-md-6 mb-sm-20">
          <div>
            <img src="./img/icons/4.png" className="mb20" alt="" />
            <h4>Быстрая поддержка</h4>
            <p>
              Dolor minim in pariatur in deserunt laboris eu pariatur labore
              excepteur cupidatat cupidatat duis dolor in.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Section;
